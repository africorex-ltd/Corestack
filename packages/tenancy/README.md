...
src/
  ...
  infrastructure/postgres/notification-payload-delivery-adapter.ts  JSON delivery payload adapter (E05-T16) — builds + durably stores a provider-ready payload from a work item; no network I/O, not wired into the processor
  postgres/                   `./postgres` package export barrel (E05-T11) — repositories, mappers, role bootstrap, RLS generators, constraint-violation helpers, notification work-item repository + consumer (E05-T14) + processing service (E05-T15) + delivery payload repository/adapter (E05-T16)
...
## Current status: scaffold (E05-T01) + Organization domain/application (E05-T02/T03) + Membership domain (E05-T04) + Invitation domain (E05-T05) + inviteMember use case (E05-T06) + acceptInvitation use case (E05-T07) + in-memory workflow integration harness (E05-T08) + Postgres schema design (E05-T09) + RLS policy design (E05-T10) + Postgres repository adapters (E05-T11) + query services (E05-T12) + HTTP interface (E05-T13) + notification orchestration (E05-T14) + notification processing service (E05-T15) + JSON delivery payload adapter (E05-T16)

What exists today:

- **JSON delivery payload adapter** (`src/application/notification-delivery-payload.ts`,
  `notification-delivery-payload-repository.ts`,
  `src/infrastructure/postgres/notification-payload-delivery-adapter.ts`,
  `postgres-notification-delivery-payload-repository.ts`, exported from
  `@corestack/tenancy/postgres`, E05-T16) — converts a `NotificationWorkItem`
  into a stable, provider-agnostic `NotificationDeliveryPayload` and
  stores it durably, still with **no network I/O**.
  `buildNotificationDeliveryPayload` is pure and, critically, **reuses the
  source work item's own `id`/`createdAt`** rather than minting fresh
  ones — the design decision that makes the payload deterministic
  (Section 3) and `store` a true idempotent upsert
  (`ON CONFLICT (id) DO NOTHING`, Section 8's "replay safety"), not just a
  plain `INSERT`. Template/subject mapping
  (`INVITATION_CREATED`→`invitation-created`, etc.) is a `Record` keyed by
  the full `NotificationWorkItemType` union, so a future fourth type
  breaks the build rather than looking up `undefined` at runtime.
  **Deliberately not an implementation of E05-T15's `NotificationDeliveryPort`** —
  that port's methods don't carry the `id`/`createdAt` this payload needs
  to stay deterministic, so this adapter takes a bare `NotificationWorkItem`
  directly instead and is **not wired into `processNextNotificationWorkItem`**
  by this task; see the design doc's "Deliberately not an implementation
  of NotificationDeliveryPort" section for the full reasoning. New table
  ...
  tenancy table uses) and a new `GlobalRepository`
  (`PostgresNotificationDeliveryPayloadRepository`, citing ADR-0026 —
  same "background adapter, no `OrgScopedContext`" reasoning as
  `PostgresNotificationWorkItemRepository`). 17 new unit tests (10 pure
  builder tests, 7 migration-consistency tests) and 8 new real-Postgres
  integration tests (durable persistence, real-column projection,
  template mapping for all three types, variable/metadata mapping,
  deterministic JSON, replay safety, `findById` miss). Full detail:
...
- A schema-only migration (`migrations/tenancy/0001_create-schema.sql`)
  ...
  (`migrations/tenancy/0002_create-tenancy-tables.sql`, E05-T10).
- 44 unit test files (496 tests) plus 1 real-Postgres integration test
  file (58 tests, `pnpm test:integration`) covering the module scaffold (compilation
  ...
  bypass) — plus 17 new delivery-payload unit tests (E05-T16: 10 pure
  builder tests, 7 migration-consistency tests) and 8 new real-Postgres
  integration tests (durable persistence, real-column projection,
  template mapping for all three types, variable/metadata mapping,
  deterministic JSON, replay safety, `findById` miss) — 58 real-Postgres
  integration tests in total (E05-T11 + E05-T12 + E05-T13 + E05-T14 +
  E05-T15 + E05-T16), 44 unit test files (496 tests) overall.
...
- **Real email delivery, external providers, and any scheduler/worker
  process.** E05-T15/T16 (see above) prove `PENDING` work items can be
  claimed, delivered, and marked reliably, and can be converted into a
  durable, provider-ready JSON payload — but still with no real delivery:
  ...
  repeatedly; `deliverNotificationWorkItemAsJsonPayload` is exported but
  not called from `processNextNotificationWorkItem` (see
  tenancy-delivery-payloads.md's "Deliberately not an implementation of
  NotificationDeliveryPort" section for why). See
  tenancy-notification-orchestration.md's "Why no email is sent yet" and
  tenancy-notification-processing.md's "Worker model" and "Future
  provider integration" sections — including the documented, accepted gap
  that a crashed worker leaves a claimed row stuck in `PROCESSING`
  forever, unresolved by this task on purpose (closing it needs the
  scheduler this task is explicitly told not to build).
...
started. Per Section 14 of the E05-T16 directive, real provider
integration is explicitly **not** to be started automatically — it waits
for an explicit E05-T17 prompt.

## See also

- [docs/modules/tenancy-delivery-payloads.md](../../docs/modules/tenancy-delivery-payloads.md) —
  the JSON delivery payload adapter (E05-T16): payload schema, why `id`/
  `createdAt` are reused from the source work item (determinism/replay
  safety), the template/variable/metadata contract, why this adapter is
  deliberately not an implementation of `NotificationDeliveryPort`, and
