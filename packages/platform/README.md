...
| Postgres `RateLimiter` adapter (fixed-window)                | ✅ E03-T41 | `PostgresRateLimiter`, `ensureRateLimitsSchema`, `pruneRateLimitWindows` (`./postgres`) — see [component spec](docs/rate-limiter.md)                                                                                                                                      |
| `CachePort` decision (no Postgres backend; Redis deferred)   | ✅ E03-T42 | No new code — in-memory `Cache` reference already shipped in kernel (E02-T07); ADR-0018 records why Postgres doesn't back a cache and why the Redis adapter is deferred                                                                                                   |
| Postgres `IdempotencyStore` adapter (request-key replay)     | ✅ E03-T43 | `PostgresIdempotencyStore`, `ensureIdempotencyKeysSchema`, `pruneIdempotencyKeys` (`./postgres`) — see [component spec](docs/idempotency-key-store.md); ADR-0019 adds the kernel port, ADR-0020 makes `organizationId` mandatory (tenant-isolation certification finding) |
...
pnpm --filter @corestack/platform test:integration     # +71 tests, real Postgres — see below
...
| --------------- | ---------------------------------------------------------------------------------------------------------- |
| Testability     | High — 191 unit tests (no database) + 71 real-Postgres integration tests across the shipped capabilities   |
