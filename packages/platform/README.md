...
| Postgres `IdempotencyStore` adapter (request-key replay)     | ✅ E03-T43 | `PostgresIdempotencyStore`, `ensureIdempotencyKeysSchema`, `pruneIdempotencyKeys` (`./postgres`) — see [component spec](docs/idempotency-key-store.md); ADR-0019 adds the kernel port itself |
